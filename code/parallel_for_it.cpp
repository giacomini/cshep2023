...
int main()
{
  ...
  for (std::size_t G : {1, 10, 100, 1'000, 10'000, 100'000, 1'000'000, 10'000'000, 100'000'000}) {
    std::atomic<int> count{};
    auto bench = benchmark([&] {
      tbb::simple_partitioner partitioner{};
      tbb::parallel_for(
          tbb::blocked_range{v.begin(), v.end(), G},
          [&](auto const& r) {
            ++count;
            std::transform(r.begin(), r.end(), r.begin(), [=](auto d) {
              for (int i{0}; i != 10; ++i) {
